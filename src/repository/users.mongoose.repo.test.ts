...
import { UserModel } from './users.mongoose.model';
import { UsersMongooseRepo } from './users.mongoose.repo';

jest.mock('./users.mongoose.model.js');

describe('Given UsersMongooseRepo repository', () => {
  ...
  describe('When the repository is instanced', () => {
    test('Then, the repo should be instance of UsersMongooseRepo;', () => {
      expect(repo).toBeInstanceOf(UsersMongooseRepo);
    });
  });
